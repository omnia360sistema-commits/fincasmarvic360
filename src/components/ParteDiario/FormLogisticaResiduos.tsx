        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
          <div className="bg-slate-900 border border-white/10 rounded-xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-5 space-y-4">
              <div><label className="block text-[10px] font-black uppercase tracking-widest text-slate-400 mb-1.5">Conductor</label><select value={formD.personal_id} onChange={e => { const id = e.target.value; const nombre = conductoresCamion.find(c => c.id === id)?.nombre ?? ''; setFormD(p => ({ ...p, personal_id: id, nombre_conductor: nombre })) }} className="w-full bg-slate-800 border border-white/10 rounded-lg px-3 py-2.5 text-sm text-white focus:border-[#6d9b7d]/50 outline-none"><option value="">— Seleccionar conductor —</option>{conductoresCamion.map(c => (<option key={c.id} value={c.id}>{c.nombre}</option>))}</select></div>
              <div><label className="block text-[10px] font-black uppercase tracking-widest text-slate-400 mb-1.5">Ganadero destino</label><select value={formD.ganadero_id} onChange={e => setFormD(p => ({ ...p, ganadero_id: e.target.value, nuevo_ganadero: '' }))} className="w-full bg-slate-800 border border-white/10 rounded-lg px-3 py-2.5 text-sm text-white focus:border-[#6d9b7d]/50 outline-none"><option value="">— Seleccionar ganadero —</option><option value="__nuevo__">+ Nuevo ganadero…</option>{ganaderos.map(g => (<option key={g.id} value={g.id}>{g.nombre}</option>))}</select>{formD.ganadero_id === '__nuevo__' && (<input type="text" placeholder="Nombre del ganadero (se guardará)" value={formD.nuevo_ganadero} onChange={e => setFormD(p => ({ ...p, nuevo_ganadero: e.target.value, ganadero_id: '__nuevo__' }))} className="mt-2 w-full bg-slate-800 border border-orange-400/40 rounded-lg px-3 py-2.5 text-sm text-white placeholder-slate-600 focus:border-orange-400/70 outline-none" />)}</div>
              <div className="grid grid-cols-2 gap-3">
                <div><label className="block text-[10px] font-black uppercase tracking-widest text-slate-400 mb-1.5">Hora salida nave</label><input type="time" value={formD.hora_salida_nave} onChange={e => setFormD(p => ({ ...p, hora_salida_nave: e.target.value }))} className="w-full bg-slate-800 border border-white/10 rounded-lg px-3 py-2.5 text-sm text-white focus:border-[#6d9b7d]/50 outline-none" /></div>
                <div><label className="block text-[10px] font-black uppercase tracking-widest text-slate-400 mb-1.5">Hora llegada ganadero</label><input type="time" value={formD.hora_llegada_ganadero} onChange={e => setFormD(p => ({ ...p, hora_llegada_ganadero: e.target.value }))} className="w-full bg-slate-800 border border-white/10 rounded-lg px-3 py-2.5 text-sm text-white focus:border-[#6d9b7d]/50 outline-none" /></div>
              </div>
              <div><label className="block text-[10px] font-black uppercase tracking-widest text-slate-400 mb-1.5">Hora regreso nave</label><input type="time" value={formD.hora_regreso_nave} onChange={e => setFormD(p => ({ ...p, hora_regreso_nave: e.target.value }))} className="w-full bg-slate-800 border border-white/10 rounded-lg px-3 py-2.5 text-sm text-white focus:border-[#6d9b7d]/50 outline-none" /></div>
