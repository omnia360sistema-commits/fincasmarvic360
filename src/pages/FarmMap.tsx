   ...
   <div className="absolute bottom-4 left-4 z-[1000] glass-strong rounded-xl px-3 py-2 space-y-1">
     {entries.map(([status, color]) => (
       <div key={status} className="flex items-center gap-2">
         ...
         <span className="text-[11px] text-foreground/80">{STATUS_LABELS[status]}</span>
       </div>
     ))}
   </div>
 );
...
export default function FarmMap() {
 ...
 const parcelIds = useMemo(() => parcels.map(p => p.properties.parcel_id), [parcels]);
     ...
     <div className="absolute top-0 left-0 right-0 z-[1000] px-4 pt-4">
       <div className="glass-strong rounded-2xl px-4 py-3 flex items-center gap-3">
         ...
         <div className="flex-1 min-w-0">
           ...
           <h1 className="text-base font-bold text-foreground truncate">{decodedFarm}</h1>
         </div>

         <span className="text-xs text-muted-foreground">{parcels.length} parcelas</span>
       </div>
     </div>
