...
type FormD = {
  nombre_conductor: string; hora_salida_nave: string
  nombre_ganadero: string; hora_llegada_ganadero: string
...
const initD = (): FormD => ({
  nombre_conductor: '', hora_salida_nave: '', nombre_ganadero: '',
  hora_llegada_ganadero: '', hora_regreso_nave: '', notas_descarga: '',
})
  ...
  async function submitD() {
    ...
    try {
      await addResiduos.mutateAsync({
        parte_id:               parteId,
        nombre_conductor:       formD.nombre_conductor       || null,
        hora_salida_nave:       timeToISO(fecha, formD.hora_salida_nave),
        nombre_ganadero:        formD.nombre_ganadero        || null,
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
          <div className="bg-slate-900 border border-white/10 rounded-xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-5 space-y-4">
              ...
              <div>
                <label className="block text-[10px] font-black uppercase tracking-widest text-slate-400 mb-1.5">Nombre conductor</label>
                <input
                  type="text"
                  value={formD.nombre_conductor}
                  onChange={e => setFormD(p => ({ ...p, nombre_conductor: e.target.value }))}
                  className="w-full bg-slate-800 border border-white/10 rounded-lg px-3 py-2.5 text-sm text-white placeholder-slate-600 focus:border-[#38bdf8]/50 outline-none"
                />
              </div>
              ...
              <div>
                <label className="block text-[10px] font-black uppercase tracking-widest text-slate-400 mb-1.5">Nombre / Nº ganadero destino</label>
                <input
                  type="text"
                  value={formD.nombre_ganadero}
                  onChange={e => setFormD(p => ({ ...p, nombre_ganadero: e.target.value }))}
                  className="w-full bg-slate-800 border border-white/10 rounded-lg px-3 py-2.5 text-sm text-white placeholder-slate-600 focus:border-[#38bdf8]/50 outline-none"
                />
              </div>
